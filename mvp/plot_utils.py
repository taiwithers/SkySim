...
cloud_freespace = {
    ...
    "MonR2": "top left",
    "M16": "top left",
    "NGC2264": "bottom right",
    "W3_Main": "bottom right",
    "W3_West": "top right",
    "NGC7538": "top right",
    "Rosette": "top right",
    "M17": "bottom left",
    "W48": "top left",
    "CygnusX_N": "bottom left",
    "CygnusX_S": "top left",
}
...
legend_kwargs = dict(
    borderaxespad=0.1,  # outside border
    borderpad=0.4,  # inside border
    framealpha=0.8,
    # labelspacing=.1,
    handlelength=1,
    fancybox=False,
...
class CustomLogLocator(ticker.Locator):
    ...
    def tick_values(self, tmin, tmax):
        ...
        if len(potential_ticks) < 4:
            return potential_ticks
...
class CustomLogFormatter(ticker.Formatter):
    ...
    def __call__(self, value, pos=None):
        ...
        if coeff != 1:
            return f"${coeff:.0f}\\times 10^{{{exponent:.0f}}}$"
        else:
...
def fig_setup(
    figsize=None, fig_only=False, wcs=None, fig_kwargs={}, plot_kwargs={}, **kwargs
):
    if figsize is None:
...
def save_fig(
    figure, title, directory=None, filetype="png", cloud=None, keep_open=False
):
...
def add_scalebar(cloud_name, ax, hdr, location=None, angular_size=5 * u.arcmin):
    ...
    arcmin_per_pc = np.tan(1 * u.pc / (CLOUD_DISTANCES[cloud_name][0] * u.kpc)).to(
        u.arcmin
    )
    ...
    wcsaxes.add_scalebar(
        ...
        color="k",
        frame=True,
        pad=0.5,  # inside border
        borderpad=0.2,  # outside border
    ...
    pixels_in_scalebar = angular_size.to(u.arcsecond) / pixel_size.to(u.arcsecond)
    return ax
    ...
    **cbar_kwargs,
    ...
    if cbar_kwargs["location"] == "right":
        if cbar_thickness is None:
            cbar_thickness = 0.07 * ax_width
        if cbar_pad is None:
            cbar_pad = cbar_thickness

        cbar_left = ax_left + ax_width + 2.5 * cbar_pad
        cbar_ax = fig.add_axes([cbar_left, ax_bottom, cbar_thickness, ax_height])
        ...
        if cbar_thickness is None:
            cbar_thickness = 0.07 * ax_height
        if cbar_pad is None:
            cbar_pad = cbar_thickness

        cbar_bottom = ax_bottom + ax_height + 2.5 * cbar_pad
        cbar_ax = fig.add_axes([ax_left, cbar_bottom, ax_width, cbar_thickness])
...
def square_contours(ax, contour_data, precision=10, filled=False, **contour_kwargs):
    fX, fY = (
        np.array(
            np.meshgrid(
                np.arange(0, contour_data.shape[1] * precision),
                np.arange(0, contour_data.shape[0] * precision),
            )
        ).astype(float)
        / precision
    )
...
def transfer_axes(
    ax, new_figure, old_figure=None, subplot_id=111, sharex=None, sharey=None
):
...
def add_diagonal(
    ...
    **plot_kwargs,
    ...
    if log:
        yvals = 10 ** (slope * np.log10(xvals) + intercept)
    else:
...
def format_unit(astropy_unit, brackets=True):
    ...
    if brackets:
        return f"$\\left({string[1:-1]}\\right)$"
    return string
